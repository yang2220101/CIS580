...
logo_img = imread('data/barcalona/images/logos/penn_engineering_logo.png');
...
logo_pts = permute(logo_pts, [2,1]);

% Load the points that the logo corners will map onto in the main image
load data/barcalona/BarcaReal_pts.mat
main_pts = permute(main_pts,[2,1,3]);
num_ima = size(main_pts, 3);
...
for i=1:num_test
    ...
    [ interior_pts ] = calculate_interior_pts(size(video_imgs{i}),...
        main_pts(:,:,test_images(i)));
    
    % Warp the interior_pts to coordinates in the logo image
    warped_logo_pts = warp_pts(main_pts(:,:,test_images(i)),...
        logo_pts,...
        interior_pts);
